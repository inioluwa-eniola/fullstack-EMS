...
const authCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!attendance?.checkOut) {
      ...
      // After 10 hours, mark attendance as checked out with the status "LATE"
      await step.sleepUntil(
        "wait-for-the-1-hour",
        new Date(new Date().getTime() + 9 * 60 * 60 * 1000),
      );
...
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (leaveApplicationId?.status === "PENDING") {
      ...
      await sendEmail({
        ...
        body: `<div style="max-width: 600px;">
            ...
            <p>Please make sure to take action on this leave application.</p>
            <br />
            <p style="font-size: 16px;">Best Regards,</p>
            <p style="font-size: 16px;">EMS</p>
          </div>`,
...
// Cron: Check attendance at 1130: AM IST (06:00 UTC) and email absent employees
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron", triggers: [{ cron: "0 0 6 * * *" }] },

  async ({ event, step }) => {
    // Step 1: Get today's date range (EST)
    const today = await step.run("get-today-date", () => {
      const startUTC = new Date(
        new Date().toLocaleDateString(
          "en-CA",
          { timeZone: "Asia/Kolkata" } + "T00:00:00+05:30",
        ),
      );
      ...
      await step.run("send-reminder-emails", async () => {
        const emailPromises = absentEmployees.map((employee) => {
          ...
          await sendEmail({
            ...
            <br />
            <p style="font-size: 14px; color: #666666">Department: ${employee.department}</p>
            <br />
